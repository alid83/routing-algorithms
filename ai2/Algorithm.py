...
class A_Star_Geometric_Algorithm:
    ...
    def _backtrack(self, curr_node):
        return [] if curr_node.parent is None else self._backtrack(curr_node.parent) + [curr_node]

    def PandX(self, array):
        array2 = []
        for i in range (len(array)-1):
            if array[i].position()[0] < array[i+1].position()[0] or array[i].position()[1] < array[i].position()[1] :
                array[i+1].parent = array[i].parent
            else:
                array2.append(array[i])


        for i in range (len(array)-1):
            if (array[i+1].position()[1] - array[i].position()[1])*(array[i+1].position()[0] - array[i].position()[0]) > 0:
                array[i+1].parent = array[i].parent
            else:
                array2.append(array[i])
    ...
    def update(self, grid):
        ...
        for step in self.get_successors(x, y):
            if self.is_valid_cell(step) and grid[step[0], step[1]] in [1,
                                                                       3] and step not in self.visited:  # 1: empty cell has not been explored yet, 3: goal cell
                self.visited.append(step)
                new_g = len(self.backtrack_solution(curr_state)) + 1
                new_h = self.heuristic(step)
                cost = new_h + new_g
                ...
                if step == self.goal_pos:
                    ...
                    solution_path2 = self.PandX(self.backtrack_solution(curr_state))
                    solution_path = self.converToXY(solution_path2)
                    break
